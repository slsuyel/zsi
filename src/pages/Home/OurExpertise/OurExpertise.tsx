...
const OurExpertise = () => {
    return (
        <div className="my-5 p-2 text-white" style={{ background: '#0d0a2b' }}>
            <div className='col-md-11 mx-auto '>
                <div className="p-3">  <h1>Our Expertise</h1>
                    <h6>Our End-To-End Global Design And Development Expertise And Broad Product Portfolio Lead To Innovative Cost-Effective Business Solutions.</h6>
                    <br />
                    <br />
                </div>
                {expertiseCategories.map((category, index) => (
                    <div key={index} className='align-items-center d-flex gap-2 mb-4'>
                        <div className='align-items-center d-flex flex-wrap gap-2 justify-content-center'>
                            ...
                            <div className='title-wrap-skill '><h6 className=''>{category.title}</h6></div>
                            {category.icons.map((icon, idx) => (
                                <div className='bg-white img-wrap rounded'> <img key={idx} src={icon} alt="" /></div>
                            ))}
                        </div>
                    </div>
                ))}
